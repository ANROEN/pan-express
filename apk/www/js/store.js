...
window.Store = (() => {
  ...
  function seed() {
    ...
    return {
      ...
      productos: [
        { id: 'marraqueta', nombre: 'Marraqueta',   precio: 1800, unidad: 'kilo', emoji: '🥖', cat: 'Tradicional', desc: 'Crujiente por fuera, suave por dentro. La clásica de la mesa chilena.', activo: true },
        { id: 'hallulla',   nombre: 'Hallulla',     precio: 1700, unidad: 'kilo', emoji: '🫓', cat: 'Tradicional', desc: 'Redonda y esponjosa, ideal para el desayuno y la once.', activo: true },
        { id: 'amasado',    nombre: 'Pan amasado',  precio: 2200, unidad: 'kilo', emoji: '🍞', cat: 'Tradicional', desc: 'Hecho a mano, sabor casero de campo.', activo: true },
        { id: 'integral',   nombre: 'Pan integral', precio: 2500, unidad: 'kilo', emoji: '🌾', cat: 'Integral',    desc: 'Con harina integral y semillas. Más fibra, más sano.', activo: true },
        { id: 'especial',   nombre: 'Pan especial', precio: 3200, unidad: 'kilo', emoji: '✨', cat: 'Especial',    desc: 'Receta de la casa con masa madre. Edición premium.', activo: true },
      ],
  ...
  /* ---------- Carga / guardado ----------
     Modo SERVIDOR: cuando la app corre con el servidor local (Node + PostgreSQL),
     los datos se guardan en la base de datos a través de /api/state.
     Modo LOCAL: si no hay servidor (p. ej. la demo en GitHub Pages o abrir el
     archivo directo), se usa localStorage del navegador como respaldo.
     El arranque nunca debe fallar. */
  let usandoServidor = false;
  let saveTimer = null;

  // Arranque inmediato (síncrono) para que las vistas funcionen al instante.
  let db;
  ...
  // Intenta conectar con el servidor/base de datos. Si responde, ese pasa a ser
  // la fuente de verdad. Se expone como Store.ready (promesa).
  const ready = (async () => {
    try {
      ...
      usandoServidor = true;
      if (j && j.data) {
        db = j.data;                       // datos reales desde PostgreSQL
      } else {
        await pushServidor();              // primera vez: siembra la base de datos
      }
      try { localStorage.setItem(KEY, JSON.stringify(db)); } catch (e) {}
      document.dispatchEvent(new CustomEvent('db:change'));
    } catch (e) {
      usandoServidor = false;              // modo local (localStorage)
    }
    return { servidor: usandoServidor };
  })();

  function save(next) {
    db = next || db;
    // Respaldo local siempre (caché)
    try { localStorage.setItem(KEY, JSON.stringify(db)); }
    catch (e) { console.warn('No se pudo guardar en localStorage; se trabaja en memoria.', e); }
    // Si hay servidor, persistir en la base de datos (con un pequeño retardo
    // para agrupar cambios seguidos).
    if (usandoServidor) {
      clearTimeout(saveTimer);
      saveTimer = setTimeout(pushServidor, 250);
    }
    document.dispatchEvent(new CustomEvent('db:change'));
  }

  async function pushServidor() {
    try {
  ...
  return {
    ...
    ESTADOS,
    get usandoServidor() { return usandoServidor; },
  };
})();
